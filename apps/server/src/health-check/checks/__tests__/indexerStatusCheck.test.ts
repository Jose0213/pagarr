...
import { IndexerStatusCheck } from "../indexerStatusCheck.js";

/** Translated from NzbDrone.Core.Test/HealthCheck/Checks/IndexerStatusCheckFixture.cs. */
...
describe("IndexerStatusCheck", () => {
  ...
  it("should_return_warning_if_indexer_unavailable", () => {
    ...
    // Within the 6-hour window -- IndexerStatusCheck (not LongTerm) cares about recent failures.
    ...
    const check = new IndexerStatusCheck(factory, statusService, new NullLocalizationService());
  ...
  it("should_return_warning_if_few_indexers_unavailable", () => {
    ...
    const blocked: IndexerStatus[] = [];
    givenIndexer(indexers, blocked, 1, 2.0, 4.0);
    givenIndexer(indexers, blocked, 2, 2.0, 4.0);
